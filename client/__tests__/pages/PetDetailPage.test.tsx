...
describe("PetDetailPage", () => {
  it("fetches and displays the details of the selected pet", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("petage").textContent).toBe(
        `Age: ${mockPet.age}`
      );
      expect(screen.getByTestId("petphone").textContent).toBe(mockPet.phone);
      const expectedCity =
        mockGeocodeData.address.city || "Location not available";
  ...
  it("should handle a 404 error for pet data", async () => {
    ...
    await waitFor(() => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error fetching your pet details:",
        expect.any(Error)
      );
    });
  });
